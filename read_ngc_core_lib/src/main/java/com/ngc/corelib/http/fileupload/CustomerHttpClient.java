...
/**
 * 使用apache的httpclient实现 （获取到的对象为单例模式）
 * 
 */
public class CustomerHttpClient {
	...
	// 指定超时时间
	private static HttpClient customerHttpClient2;

	private CustomerHttpClient() {
	}

	/**
	 * 创建一个新HttpClient对象（限新浪微博分享专用）
	 * 
	 * @return
	 */
	...
	public static synchronized HttpClient getHttpClient(int timeout) {
		if (null == customerHttpClient2) {
			customerHttpClient2 = createHttpClient(timeout);
		}
		return customerHttpClient2;
	}
	...
	private static DefaultHttpClient createHttpClient(int timeout) {
		...
		if (timeout < 0) {
			/* 从连接池中取连接的超时时间 */
			ConnManagerParams.setTimeout(params, 1000 * 30);
			/* 连接超时 */
