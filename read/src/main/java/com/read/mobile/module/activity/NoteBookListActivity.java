...
import com.read.mobile.module.adapter.BookNoteListAdapter;

public class NoteBookListActivity extends BaseActivity implements IXListViewListener {

	private XListView helpLV;
	private List<BookItem> beans = new ArrayList<>();
	private BookNoteListAdapter adapter;

	private BookNoteListResult favorListResult;

	@Override
	protected void initData() {
		super.initData();
		favorListResult = (BookNoteListResult) getIntent().getSerializableExtra("BookNoteListResult");
	...
	protected void initViwes() {
		...
		adapter = new BookNoteListAdapter(this, beans);
		...
		helpLV.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
				// bookInfo(((BookItem)
				// parent.getAdapter().getItem(position)).getIsbn());
			}
		});
	}
		...
		NetAsyncTask asyncTask = new NetAsyncTask(BookNoteListResult.class, new AsyncTaskListener() {
			...
			public void onTaskSuccess(int requestId, BaseResult result) {
				...
				if (result.getHead().getCode().equals(Contacts.RESULT_CODE_OK)) {
					// TODO
					BookNoteListResult listResult = (BookNoteListResult) result;
					...
					Tips.tipLong(NoteBookListActivity.this, result.getHead().getMessage());
				}
			}
			...
			public void onTaskFail(int requestId, Exception e) {
				...
				helpLV.stopLoadMore();
				helpLV.stopRefresh();
				Tips.tipLong(NoteBookListActivity.this, e.getLocalizedMessage());
			}
		}, request);
	...
	private void bookInfo(String isbn) {
		...
		NetAsyncTask asyncTask = new NetAsyncTask(BookInfoResult.class, new AsyncTaskListener() {
			...
			public void onTaskSuccess(int requestId, BaseResult result) {
				dismissProgressDialog();
				if (result.getHead().getCode().equals(Contacts.RESULT_CODE_OK)) {
					Intent intent = new Intent(NoteBookListActivity.this, BookDetailActivity.class);
