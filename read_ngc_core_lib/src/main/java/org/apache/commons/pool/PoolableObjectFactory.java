 ...
 * <li>
 * {@link #activateObject activateObject} is invoked on every instance that has been {@link #passivateObject passivated} before it is
 * {@link ObjectPool#borrowObject borrowed} from the pool.</li>
 * <li>
 * {@link #validateObject validateObject} is invoked on {@link #activateObject activated} instances to make sure they can be {@link ObjectPool#borrowObject
 * borrowed} from the pool. <code>validateObject</code> <strong>may</strong> also be used to test an instance being {@link ObjectPool#returnObject returned} to
 * the pool before it is {@link #passivateObject passivated}. It will only be invoked on an activated instance.</li>
 * <li>
 ...
 * <code>validateObject</code>, or for reasons specific to the pool implementation.) There is no guarantee that the instance being destroyed will be considered
 * active, passive or in a generally consistent state.</li>
 * </ol>
 * </p>
 * <p>
 * {@link PoolableObjectFactory} must be thread-safe. The only promise an {@link ObjectPool} makes is that the same instance of an object will not be passed to
 * more than one method of a <code>PoolableObjectFactory</code> at a time.
 * </p>
 * 
 * @param <T>
...
public interface PoolableObjectFactory<T> {
	/**
	 * Creates an instance that can be served by the pool. Instances returned from this method should be in the same state as if they had been
	 * {@link #activateObject activated}. They will not be activated before being served by the pool.
	 * 
	 ...
	 * <p>
	 * It is important for implementations of this method to be aware that there is no guarantee about what state <code>obj</code> will be in and the
	...
	/**
	 * Ensures that the instance is safe to be returned by the pool. Returns <code>false</code> if <code>obj</code> should be destroyed.
	 * 
	 * @param obj
